...
from continuous_controls_monitoring.adapters._review_payload import (
    result_to_review,
)
...
def test_the_payload_that_reaches_the_wire_is_redacted_whichever_family_built_it() -> None:
    ...
    demo.
    """
    result = sample_cases.PII_RESULT
    payload = repr(
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    ...
    service = build_monitoring_service(local)
    ...
    assert monitored.result.citations, "an offline answer must still be cited"
    assert monitored.review_ref, "an escalation must be routed, not merely flagged"

    onprem = build_container(local_settings(profile="onprem"))
    onprem_service = build_monitoring_service(onprem)
    with pytest.raises(NotImplementedError):
        onprem_service.evaluate_pack(
            pack, as_of=sample_cases.AS_OF, tenant=sample_cases.TENANT, actor=sample_cases.ACTOR
        )
