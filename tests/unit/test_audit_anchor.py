...
from continuous_controls_monitoring.adapters.local.audit import (
    LocalAuditAdapter,
)
...
from tests.conftest import build_monitoring_service
from tests.fixtures import sample_cases

_ANCHOR_ENV = "CCM_AUDIT_ANCHOR"
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    # A full run records one audit event per pack, which is more than three, so the anchored
    # chain has a real tail to truncate.
    service.run(as_of=sample_cases.AS_OF, tenant=sample_cases.TENANT, actor=sample_cases.ACTOR)
    return audit
