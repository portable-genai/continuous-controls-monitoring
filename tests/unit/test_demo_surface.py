...
def test_the_demo_data_is_obviously_fictional() -> None:
    run = demo.DemoRun()
    run.run_to_end()
    blob = json.dumps(run.state(), sort_keys=True) + demo.ACTOR + demo.PLANTED_NRIC
    for real_looking in ("@gmail.", "@outlook.", "@yahoo."):
        assert real_looking not in blob
    assert ".example" in demo.ACTOR, "the demo actor should use an .example domain"
    # The planted identifier never survives into the immutable record (redact-before-write).
    redaction = next(step for step in run.state()["steps"] if step["key"] == "redaction")
    assert redaction["facts"]["planted_identifier_leaked"] is False
