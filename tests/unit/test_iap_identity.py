...
from continuous_controls_monitoring.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from continuous_controls_monitoring.config import Settings
from continuous_controls_monitoring.ports.identity import (
    VERIFIED,
...
_API_MODULE = "continuous_controls_monitoring.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        "  control_inventory:",
        *[
            f"    {p}: {_PKG}.adapters.local.control_inventory:LocalControlInventory"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.control_inventory:OnPremControlInventory",
        "  evidence_scanner:",
        *[
            f"    {p}: {_PKG}.adapters.local.evidence_scanner:LocalEvidenceScanner"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.evidence_scanner:OnPremEvidenceScanner",
        "  control_evidence:",
        *[
            f"    {p}: {_PKG}.adapters.local.control_evidence:LocalControlEvidence"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.control_evidence:OnPremControlEvidence",
        "  writeback:",
        *[
            f"    {p}: {_PKG}.adapters.local.writeback:LocalEffectivenessWriteback"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.writeback:OnPremEffectivenessWriteback",
        "  timeseries:",
        *[
            f"    {p}: {_PKG}.adapters.local.timeseries:LocalTimeSeriesExport"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.timeseries:OnPremTimeSeriesExport",
        "  generation:",
        *[
            f"    {p}: {_PKG}.adapters.local.generation:LocalNarrationGenerator"
            for p in ("local", "gcp")
        ],
...
def test_an_emptied_audience_variable_refuses_at_load(
    ...
    ``${CCM_IAP_AUDIENCE:-}`` is expanded by the settings loader, and the loader refuses a
