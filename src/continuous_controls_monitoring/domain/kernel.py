...
def redacted_citations(citations: tuple[Citation, ...]) -> tuple[Citation, ...]:
    ...
    A snippet is a slice of its source and a locator is routinely built from one (an evidence
    URI, a control subject), so both are raw upstream text wearing a structural-looking name. No
    sink can tell an engine-built citation from an evidence-built one by inspection, so this
    masks unconditionally: redacting text that carries no identifier is a no-op, while deciding
    per caller is how one caller ends up forgetting.
    """
    return tuple(
        Citation(
...
class AuditEvent:
    ...
    that masked ``redacted_summary`` and passed its citations through untouched wrote whatever
    they carried into the WORM record anyway, one field away, into a record that is immutable
    and long-retained by design. So construction masks every CONTENT field: the summary, and
    each citation's locator, title and snippet. Redaction is idempotent, so a caller that
    already redacted loses nothing by the second pass.
