"""Local EffectivenessWritebackPort: an in-memory Rgc7 graph stand-in that actually records.

Appends a result as an evidence node linked to the tested control, exactly as the managed
adapter appends it to Rgc7's graph, and refuses a write-back naming a control the inventory does
not contain. It is deliberately not a no-op: a silent local writer would let the gate pass while
the write-back path proved nothing, so this one stores what it was given and exposes it for the
"a written result is readable back, attached to the control" assertion.
"""
...
#: silently created, so Aud2 cannot mint a control by writing an evidence node to it.
    ...
    def append_result(self, result: ControlTestResult, *, tenant: str = "") -> str:
        if result.control_id not in _KNOWN_CONTROL_IDS:
            raise UnknownControlError(
                f"write-back names control {result.control_id!r}, which is not in the Rgc7 "
