...
One env var (``CCM_PROFILE``) selects the adapter family for every
...
_PROFILE_ENV = "CCM_PROFILE"
_SETTINGS_ENV = "CCM_SETTINGS"
...
#: The networked profiles: the ones that must name what an on control calls, checked at boot.
...
class ProfileChoice:
    ...
    The variable is ``CCM_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("CCM_PROFILE", "local")``: that fallback reads an
...
_PKG = "continuous_controls_monitoring"
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "control_inventory": {
        "local": f"{_PKG}.adapters.local.control_inventory:LocalControlInventory",
        "gcp": f"{_PKG}.adapters.gcp.control_inventory:RemoteControlInventory",
        "onprem": f"{_PKG}.adapters.onprem.control_inventory:OnPremControlInventory",
    },
    "evidence_scanner": {
        "local": f"{_PKG}.adapters.local.evidence_scanner:LocalEvidenceScanner",
        "gcp": f"{_PKG}.adapters.gcp.evidence_scanner:CloudEvidenceScanner",
        "onprem": f"{_PKG}.adapters.onprem.evidence_scanner:OnPremEvidenceScanner",
    },
    "control_evidence": {
        "local": f"{_PKG}.adapters.local.control_evidence:LocalControlEvidence",
        "gcp": f"{_PKG}.adapters.gcp.control_evidence:RemoteControlEvidence",
        "onprem": f"{_PKG}.adapters.onprem.control_evidence:OnPremControlEvidence",
    },
    "writeback": {
        "local": f"{_PKG}.adapters.local.writeback:LocalEffectivenessWriteback",
        "gcp": f"{_PKG}.adapters.gcp.writeback:RemoteEffectivenessWriteback",
        "onprem": f"{_PKG}.adapters.onprem.writeback:OnPremEffectivenessWriteback",
    },
    "timeseries": {
        "local": f"{_PKG}.adapters.local.timeseries:LocalTimeSeriesExport",
        "gcp": f"{_PKG}.adapters.gcp.timeseries:BigQueryTimeSeriesExport",
        "onprem": f"{_PKG}.adapters.onprem.timeseries:OnPremTimeSeriesExport",
    },
    "generation": {
        "local": f"{_PKG}.adapters.local.generation:LocalNarrationGenerator",
        "gcp": f"{_PKG}.adapters.gcp.generation:VertexNarrationGenerator",
        "onprem": f"{_PKG}.adapters.onprem.generation:OnPremNarrationGenerator",
    },
}
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``CCM_SETTINGS``) that does not
...
def _policy_from(data: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    An absent block is the unset state and takes the shipped default packs downstream; a present
    block is honoured as written, including an empty pack list an operator deliberately set.
    """
...
REVIEW_ROUTING_ENV = "CCM_REVIEW_ROUTING"
...
class Settings:
    ...
    #: The parsed ``policy:`` block: the bank-owned control-test packs and their thresholds. An
    #: empty mapping means no policy was configured, so the shipped default packs apply.
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            rgc7_read_url=str(data.get("rgc7_read_url") or ""),
            rgc7_writeback_url=str(data.get("rgc7_writeback_url") or ""),
            rsk1_evidence_url=str(data.get("rsk1_evidence_url") or ""),
...
class Container:
    ...
    @cached_property
    def control_inventory(self) -> ControlInventoryPort:
        adapter = self._bind("control_inventory")
        assert isinstance(adapter, ControlInventoryPort)
        return adapter

    @cached_property
    def evidence_scanner(self) -> EvidenceScannerPort:
        adapter = self._bind("evidence_scanner")
        assert isinstance(adapter, EvidenceScannerPort)
        return adapter

    @cached_property
    def control_evidence(self) -> ControlEvidencePort:
        adapter = self._bind("control_evidence")
        assert isinstance(adapter, ControlEvidencePort)
        return adapter

    @cached_property
    def writeback(self) -> EffectivenessWritebackPort:
        adapter = self._bind("writeback")
        assert isinstance(adapter, EffectivenessWritebackPort)
        return adapter

    @cached_property
    def timeseries(self) -> TimeSeriesExportPort:
        adapter = self._bind("timeseries")
        assert isinstance(adapter, TimeSeriesExportPort)
        return adapter
