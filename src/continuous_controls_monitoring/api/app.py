...
``CCM_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``CCM_S2S_TOKEN`` is wrong in the most dangerous direction:
...
_TOKEN_ENV = "CCM_S2S_TOKEN"
_INSECURE_DEMO_ENV = "CCM_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "CCM_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "CCM_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "CCM_S2S_AUDIENCE"
_API_HOST_ENV = "CCM_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``CCM_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``CCM_PROFILE`` must never be allowed to
    ...
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "CCM_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "continuous-controls-monitoring"
...
_APP_TITLE = "Continuous Controls Monitoring"
...
#
# Note what is NOT in this expression: CCM_S2S_TOKEN. A service
...
def _tenant_of(principal: Principal, container: Container) -> str:
    return principal.tenant or container.settings.tenant


@app.post("/v1/controls/test", response_model=ControlTestResponse, tags=["artifacts"])
def test_control(
    request: TestControlRequest,
