...
The catalog id (``Aud2``) is left alone unless ``--catalog-id`` is given: it names the entry in
...
_OLD_PACKAGE = "continuous_controls_monitoring"
_OLD_ENV_PREFIX = "CCM_"
_OLD_BARE_PREFIX = "CCM"
_OLD_RESOURCE = "aud2-svc"
_OLD_DIST = "continuous-controls-monitoring"
_OLD_CATALOG_ID = "Aud2"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (CCM_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource",
        required=True,
        help="new cloud resource stem (Terraform name_prefix), e.g. acme-ccm",
    )
