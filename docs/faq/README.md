...
| [features-faq.md](features-faq.md) | Product / audit / delivery | what the grading engine decides, what the model is allowed to say, and the boundary with `obligations-control-mapping` and `compliance-advisory` |
...
catalog. Where a concern belongs to another repo (the control inventory `obligations-control-mapping`, the cloud
control-evidence packs `compliance-advisory`, the guardrail gateway `agent-guardrail-gateway`, the human-review console `human-review-console`, the eval
